...
class Level6: SKScene {
    var enemy1lives = 1
    var enemy2lives = 1
    var enemy3lives = 1
    var doorisopened = false
    var enemy1died = false
    var enemy2died = false
    var enemy3died = false
    var playerlives = 3
    var enemyhit = false
    ...
    override func didMove(to view: SKView) {
        ...
        enemy1 = (childNode(withName: "enemy 1") as! SKSpriteNode)
        enemy2 = (childNode(withName: "enemy 2") as! SKSpriteNode)
        enemy3 = (childNode(withName: "enemy 3") as! SKSpriteNode)
...
extension Level6: SKPhysicsContactDelegate{
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if contactMask == PhysicsCategory.player | PhysicsCategory.enemy {
            if playerlives > 0
            {
                ...
                if let node = contact.bodyA.node?.name == "enemy 1" ? contact.bodyA.node : contact.bodyB.node
                {
                    if node.name == "enemy 1"
                    {
                ...
                if let node3 = contact.bodyA.node?.name == "enemy 2" ? contact.bodyA.node : contact.bodyB.node
                {
                    if node3.name == "enemy 2"
                    {
                    enemy2lives = enemy2lives - 1
                    if enemy2lives == 0
                    {
                    node3.run(SKAction.removeFromParent())
                        enemy2died = true
                    }
                    else
                    {
                        mental(object1: (node2) as! SKSpriteNode, object2: (node3) as! SKSpriteNode)
                    }
                    }
                }
                if let node4 = contact.bodyA.node?.name == "enemy 3" ? contact.bodyA.node : contact.bodyB.node
                {
                    if node4.name == "enemy 3"
                    {
                    enemy3lives = enemy3lives - 1
                    if enemy3lives == 0
                    {
                    node4.run(SKAction.removeFromParent())
                        enemy3died = true
                        
                    }
                    else
                    {
                        mental(object1: (node2) as! SKSpriteNode, object2: (node4) as! SKSpriteNode)
                    }
                    }
                }
